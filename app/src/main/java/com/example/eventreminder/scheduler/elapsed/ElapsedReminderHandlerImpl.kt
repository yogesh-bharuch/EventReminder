...
class ElapsedReminderHandlerImpl @Inject constructor(
    ...
    override suspend fun handleIfElapsed(
        ...
        Timber.tag(TAG).d(
            "ELAPSED | ElapsedReminderHandler::%s - Evaluating id=%s enabled=%s deleted=%s repeatRule=%s eventAt=%d now=%d",
            FN_HANDLE,
            reminder.id,
            reminder.enabled,
            reminder.isDeleted,
        ...
        if (reminder.isDeleted) {
            Timber.tag(TAG).d(
                "ELAPSED | ElapsedReminderHandler::%s - Skip (deleted) id=%s",
                FN_HANDLE,
                reminder.id
            )
            return false
        ...
        if (!reminder.enabled) {
            Timber.tag(TAG).d(
                "ELAPSED | ElapsedReminderHandler::%s - Skip (already disabled) id=%s",
                FN_HANDLE,
                reminder.id
            )
            return false
        ...
        if (reminder.repeatRule != null) {
            Timber.tag(TAG).d(
                "ELAPSED | ElapsedReminderHandler::%s - Skip (repeating rule=%s) id=%s",
                FN_HANDLE,
                reminder.repeatRule,
                reminder.id
            )
        ...
        if (reminder.eventEpochMillis >= nowEpochMillis) {
            Timber.tag(TAG).d(
                "ELAPSED | ElapsedReminderHandler::%s - Skip (not elapsed) id=%s eventAt=%d",
                FN_HANDLE,
                reminder.id,
                reminder.eventEpochMillis
            )
        ...
        // --- action: disable ---
        Timber.tag(TAG).i(
            "ELAPSED | ElapsedReminderHandler::%s - Disabling one-time reminder id=%s eventAt=%d now=%d",
            FN_HANDLE,
            reminder.id,
            reminder.eventEpochMillis,
            nowEpochMillis
        ...
        Timber.tag(TAG).i(
            "ELAPSED | ElapsedReminderHandler::%s - Disabled successfully id=%s",
            FN_HANDLE,
            reminder.id
        )
