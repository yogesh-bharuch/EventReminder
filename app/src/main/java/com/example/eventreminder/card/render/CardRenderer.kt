 ...
 * CardRenderer
 *
 * Simple bitmap renderer for CardRenderRequest.
 * Current capabilities:
 * - Background color
 * - Simple accent gradient
 * - White rounded card panel
 * - Text blocks using TextLayoutEngine (wrap, auto-fit, center)
 * - Relation icon (optional)
 *
 * TODO steps will extend: Photo layer, Stickers, Advanced themes.
 */
object CardRenderer {
    ...
    /**
     * Render the given CardRenderRequest into a Bitmap.
     */
    fun renderToBitmap(context: Context, req: CardRenderRequest): Bitmap {
        ...
        // =============================================================
        // Accent Gradient (simple vertical)
        // =============================================================
        try {
        ...
        // =============================================================
        // Inner White Card Panel (Rounded)
        // =============================================================
        ...
        req.photoLayer?.let { layer ->
            ...
            val photoUri = layer.photoUri   // ← capture into local read-only val

            if (photoUri != null) {

                val targetSize = layer.size.toInt()

                // Load bitmap using the safe local value
                val loaded = PhotoLoader.loadBitmap(context, photoUri, targetSize)
                ...
                if (loaded != null) {
                    ...
                    Timber.tag(TAG).w("PhotoLayer present but load failed: $photoUri")
                }
            }
        }

        // =============================================================
        // Text Blocks Rendering
        // =============================================================
    ...
    /**
     * Render → Save PNG → Return content:// Uri
     */
    suspend fun renderAndSaveToGallery(context: Context, req: CardRenderRequest): android.net.Uri? {
        ...
        try {
            ...
            resolver.openOutputStream(outUri).use { out ->
                ...
                if (!success) Timber.tag(TAG).w("Bitmap.compress returned false")
            }

            Timber.tag(TAG).d("Saved card PNG: $outUri")
